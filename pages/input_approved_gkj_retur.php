...
// Direktori upload dokumen retur
$uploadDir = realpath(__DIR__ . '/../uploads') !== false ? (__DIR__ . '/../uploads/retur/') : (__DIR__ . '/../uploads/retur/');
if (!is_dir($uploadDir)) { @mkdir($uploadDir, 0777, true); }

// Helper: daftar file yang sudah terupload untuk id_nsp terkait
function listReturFiles($id_nps)
{
    $dir = __DIR__ . '/../uploads/retur/';
    $list = [];
    if (is_dir($dir)) {
        $dh = opendir($dir);
        if ($dh) {
            while (($f = readdir($dh)) !== false) {
                if ($f === '.' || $f === '..') continue;
                if (strpos($f, 'retur_' . intval($id_nps) . '_') === 0) {
                    $list[] = $f;
                }
            }
            closedir($dh);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (count($selectedIds) === 0) {
        ...
        // Cek apakah ada data yang sudah pernah diupdate (sudah ada approved_gkj/personil_gkj/tgl_approved_gkj)
        $condApproved = "((approved_gkj IS NOT NULL AND approved_gkj <> '') OR (personil_gkj IS NOT NULL AND personil_gkj <> '') OR (tgl_approved_gkj IS NOT NULL AND tgl_approved_gkj <> '0000-00-00 00:00:00'))";
        ...
        $already = [];
        if ($resCheck) {
            while ($rck = mysqli_fetch_assoc($resCheck)) { $already[] = $rck['id']; }
        }
        if (count($already) > 0) {
        ...
        $sql = "UPDATE tbl_detail_retur_now SET approved_gkj='$pilihan', personil_gkj='$personil_gkj', tgl_approved_gkj=NOW() WHERE id IN ($idsSql)$whereExtra";
        if (mysqli_query($con, $sql)) {
            $affected = mysqli_affected_rows($con);
            $message = '<div class="alert alert-success">Berhasil update ' . intval($affected) . ' data.</div>';
            // Kirim email ke ms emley
            $to = ['aftersales.adm@indotaichen.com','arif.efendi@indotaichen.com','emily.lai@indotaichen.com'];  // contoh penerima
            $subject = 'Status Bon Retur #' . htmlspecialchars($bon['no_retur']) . '';
            $baseUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://" . $_SERVER['HTTP_HOST'];
            $linkApproveCSR = $baseUrl . "/csr-itti/pages/input_approved_csr_retur.php?no_order=" . urlencode($no_order)
                . "&po=" . urlencode($po)
                . "&id_nsp=" . urlencode($id_nps)
                . "&id_cek=" . urlencode($id_cek)
                . "&id_cek1=" . urlencode($id_cek1)
                . "&id_cek2=" . urlencode($id_cek2);
            $bodyHtml = 'Status approve GKJ untuk Retur <b>#' . htmlspecialchars($bon['no_retur']) . '</b> telah diinput untuk ' . count($selectedIds) . ' data:<br>';
...
<body>
    <div class="container" style="margin-top:30px; max-width:1100px;">
        <div class="row">
            <div class="col-md-4">
                <h3 style="margin-top:0;">Status Bon Retur GKJ: <span style="color:blue;">#<?php echo htmlspecialchars($bon['no_retur']); ?></span></h3>
                <?php echo $message; ?>
                <form method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        <label for="stok">Status Approve GKJ</label>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="nama_lengkap" name="nama_lengkap" required>
                            ...
                            <?php
                            $qKep = mysqli_query($con, "SELECT nama, email FROM master_email WHERE departemen = 'GKJ' ORDER BY nama ASC");
