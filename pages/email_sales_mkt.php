...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($cek['approved_rmp'])) {
        $message = '<div class="alert alert-warning">Status sudah pernah mengirimkan email notifikasi.</div>';
    } else {
        ...
        $subject = 'Status Bon Ganti Kain #' . htmlspecialchars($bon['no_bon']) . '';
        $baseUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://" . $_SERVER['HTTP_HOST'];
        $linkInputStok = $baseUrl . "/csr-itti/pages/input_approve_mkt.php?id=" . urlencode($id);
        $linkemail = $baseUrl . "/csr-itti/pages/input_approve_rmp.php?id=" . urlencode($id);
        $linkCetakAbs = $baseUrl . "/csr-itti/pages/" . $linkCetak;

        // Bagian pembuka email (dipakai untuk semua penerima)
        $bodyIntro = 'Status stok untuk Bon <b>#' . htmlspecialchars($bon['no_bon']) . '</b> telah diinput Dept MKT:<br>';
        $bodyIntro .= '<b>Keterangan:</b> ' . htmlspecialchars($bon['approved_mkt']) . '<br>';
       ...
       $bodyForOthers = $bodyIntro
            . 'Lihat detail atau input status di: <strong>“Mohon mengingatkan agar Kepala Departemen bisa segera review dan Approved”</strong><br>'
            . "<a href='" . $linkCetakAbs . "' target='_blank' style='color: #337ab7; text-decoration: underline;'>Cek Status Bon</a><br>"
            . $detailHtml;

        $bodyForHead = $bodyIntro
            . 'Lihat detail bon di: '
            . "<a href='" . $linkemail . "' target='_blank' style='color: #337ab7; text-decoration: underline;'>Input Status Bon Kain RMP</a><br>"
        ...
        foreach ($toUnique as $addr) {
            if (strcasecmp($addr, $kepalaEmail) !== 0) {
                $toOthers[] = $addr;
            }
        }
        ...
        if ($okOthers) {
            $sentList = $toOthers;
            $sentList[] = $kepalaEmail;
            // Update waktu Sales/MKT kirim ke RMP
            $query = "UPDATE tbl_ganti_kain_now SET tgl_send_mkt_to_rmp=NOW() WHERE id=$id";
...
<head>
    <meta charset="utf-8">
    <title>Notifikasi MKT - Bon Ganti Kain</title>
...
<body>
    <div class="container" style="margin-top:30px; max-width:1100px;">
        <div class="row">
            <div class="col-md-4">
                <h3 style="margin-top:0;">Kirim Email: <span style="color:blue;">#<?php echo htmlspecialchars($bon['no_bon']); ?></span></h3>
                <?php echo $message; ?>
                <form method="post">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="email" name="email[]" multiple required>
                            ...
                            <?php
                            $queryEmail = mysqli_query($con, "SELECT email, nama, departemen FROM master_email WHERE departemen = 'RMP'  ORDER BY nama ASC");
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="kepala_email" name="kepala_email" >
                            <option value="" disabled selected>-- Pilih --</option>
                            <?php
                            $qKep = mysqli_query($con, "SELECT email, nama, departemen FROM master_email WHERE departemen = 'RMP' ORDER BY nama ASC");
