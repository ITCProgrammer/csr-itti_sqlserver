...
// Helper kirim email (samakan pola dengan input_approved_adm_ppc_retur.php)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (count($selectedIds) === 0) {
        ...
        $condApproved = "((approved_mkt IS NOT NULL AND approved_mkt <> '') OR (personil_mkt IS NOT NULL AND personil_mkt <> '') OR (tgl_approved_mkt IS NOT NULL AND tgl_approved_mkt <> '0000-00-00 00:00:00'))";
        ...
        if (count($already) > 0) {
            ...
            $sql = "UPDATE tbl_detail_retur_now SET approved_mkt='$pilihan', personil_mkt='$personil', tgl_approved_mkt=NOW() WHERE id IN ($idsSql)$whereExtra";
            if (mysqli_query($con, $sql)) {
                ...
                if (strtolower($pilihan) === 'approved') {
                    // Gunakan penerima email sama dengan input_approve_mkt.php
                    $to = ['aftersales.adm@indotaichen.com'];

                    // Info dasar
                    $noRetur = isset($bon['no_retur']) ? $bon['no_retur'] : '';
                    $subject = 'Approved MKT Retur #' . htmlspecialchars($noRetur);
                    $baseUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'];
                    $linkPage = $baseUrl . '/csr-itti/pages/' . $linkCetak; // gunakan link cetak yang sudah dirangkai

                    // Rangkai body ringkas
                    $bodyHtml = 'Status Retur <b>#' . htmlspecialchars($noRetur) . '</b> telah di-<b>Approved</b> oleh MKT.<br>';
...
<head>
    <meta charset="utf-8">
    <title>Approved MKT Retur</title>
...
<body>
    <div class="container" style="margin-top:30px; max-width:1100px;">
        <div class="row">
            <div class="col-md-4">
                <h3 style="margin-top:0;">Status Bon Retur MKT: <span style="color:blue;">#<?php echo htmlspecialchars($bon['no_retur']); ?></span></h3>
                <?php echo $message; ?>
                <form method="post">
                    <div class="form-group">
                        <label for="stok">Status Approve MKT</label>
