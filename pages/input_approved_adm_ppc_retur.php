...
// Helper kirim email (samakan pola dengan input_approve_mkt.php)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (count($selectedIds) === 0) {
        ...
        $condApproved = "((approved_ppc IS NOT NULL AND approved_ppc <> '') OR (personil_ppc IS NOT NULL AND personil_ppc <> '') OR (tgl_approved_ppc IS NOT NULL AND tgl_approved_ppc <> '0000-00-00 00:00:00'))";
        ...
        if (count($already) > 0) {
            ...
            $sql = "UPDATE tbl_detail_retur_now SET approved_ppc='$pilihan', personil_ppc='$personil', tgl_approved_ppc=NOW() WHERE id IN ($idsSql)$whereExtra";
            if (mysqli_query($con, $sql)) {
                ...
                if (strtolower($pilihan) === 'approved') {
                    ...
                    $subject = 'Approved PPC Retur #' . htmlspecialchars($noRetur);
                    $baseUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'];
                    $linkPage = $baseUrl . '/csr-itti/pages/' . $linkCetak; // gunakan link cetak yang sudah dirangkai

                    // Rangkai body ringkas
                    $bodyHtml = 'Status Retur <b>#' . htmlspecialchars($noRetur) . '</b> telah di-<b>Approved</b> oleh PPC.<br>';
...
<head>
    <meta charset="utf-8">
    <title>Approved PPC Retur</title>
...
<body>
    <div class="container" style="margin-top:30px; max-width:1100px;">
        <div class="row">
            <div class="col-md-4">
                <h3 style="margin-top:0;">Status Bon Retur PPC: <span style="color:blue;">#<?php echo htmlspecialchars($bon['no_retur']); ?></span></h3>
                <?php echo $message; ?>
                <form method="post">
                    <div class="form-group">
                        <label for="stok">Status Approve PPC</label>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="nama_lengkap" name="nama_lengkap" required>
                            ...
                            <?php
                            $qKep = mysqli_query($con, "SELECT nama, email FROM master_email WHERE departemen = 'PPC' ORDER BY nama ASC");
